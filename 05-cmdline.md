...
Since we did not specify a filename for writing, `cat` sends the output to [standard output](reference.html#standard-output-stdout),
...
R has a package named [argparse](https://cran.r-project.org/package=argparse) that helps handle complex command-line flags (it utilizes a [Python module](https://docs.python.org/dev/library/argparse.html) of the same name).
We will not cover this package in this lesson but when you start writing programs with multiple parameters you'll want to read through the package's [vignette](https://cran.r-project.org/package=argparse/vignettes/argparse.html).
...
If you ran this, you can pause R by typing <kbd>Ctrl</kbd>\+<kbd>Z</kbd> (technically it is still paused in the background; if you want to fully kill the process type `kill %`; see [bash manual](https://www.gnu.org/software/bash/manual/bash.html#Job-Control-Basics) for more information).
